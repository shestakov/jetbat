...
{
	public class StoredQueryDefinitionSqlLoader : IObjectDefinitionSqlLoader
	{
		private readonly string _connectionString;
		private readonly bool _enableCaching;
		private const int PlainObjectListViewObjectTypeID = 4;

		public StoredQueryDefinitionSqlLoader(string connectionString, bool enableCaching)
		{
		...
		{
			return new StoredQueryDefinition((StoredQuery)Load(objectNamespace, objectName));
		}

		public BusinessObject Load(string objectNamespace, string objectName)
		{
			string cacheDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, StaticHelper.MetadataCacheDirectoryName + @"\StoredQuery");
			...
			if (_enableCaching)
			{
				...
				{
					XmlSerializer serializer = new XmlSerializer(typeof(StoredQuery));
					using (FileStream stream = new FileStream(cacheFileName, FileMode.Open, FileAccess.Read))
						return (StoredQuery)serializer.Deserialize(stream);
				}
			}
			...
			{
				sqlConnection.Open();
				SqlCommand command = new SqlCommand("MetadataEngine_LoadStoredQueryDescriptor", sqlConnection)
			...
			BusinessObject definition = new StoredQuery
				{
