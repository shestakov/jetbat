...
CREATE PROCEDURE [dbo].[MealCalc_CalcDayMenu_ClearWriteOff]
	@ID INT
AS
DECLARE @calcDayID INT
DECLARE @calcDayDate DATETIME
DECLARE @error_message varchar(1000)
DECLARE @TableIncomesToRollback TABLE (ID INT)

INSERT INTO @TableIncomesToRollback (ID)
SELECT DISTINCT dbo.MealCalc_CalcDayMenuGoodSpendWriteOff.GoodIncomeID FROM dbo.MealCalc_CalcDayMenuGoodSpendWriteOff
...
UPDATE dbo.MealCalc_GoodIncome SET
	dbo.MealCalc_GoodIncome.QuantityWrittenOff = ISNULL(dbo.MealCalc_GoodIncome.QuantityWrittenOff, 0) - 
		ISNULL((
			SELECT SUM(dbo.MealCalc_CalcDayMenuGoodSpendWriteOff.Quantity)
			FROM dbo.MealCalc_CalcDayMenuGoodSpendWriteOff
			WHERE dbo.MealCalc_CalcDayMenuGoodSpendWriteOff.GoodIncomeID = dbo.MealCalc_GoodIncome.ID
		), 0)
WHERE dbo.MealCalc_GoodIncome.ID IN (SELECT ID FROM @TableIncomesToRollback)
...
--		dbo.MealCalc_GoodIncome.Quantity > ISNULL(MealCalc_GoodIncome.QuantityWrittenOff, 0)
