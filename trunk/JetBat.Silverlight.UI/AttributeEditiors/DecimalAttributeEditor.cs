...
{
	public class DecimalAttributeEditor : ObjectAttributeEditor
	{
		readonly NumericUpDown numericUpDown = new NumericUpDown();
		private readonly Button buttonSetNull;

		public DecimalAttributeEditor(ObjectAttribute attribute)
			: base(attribute)
		{
		...
		{
			get { return isNull ? (object) null : Convert.ToDecimal(numericUpDown.Value); }
			set
			{
				if (value == null)
				{
